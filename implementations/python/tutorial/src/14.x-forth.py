'''
We're just about finished implementing strings, but to finish up here in part 14, we'll add support for escaped characters inside strings. Currently if you had something like this:
    "my name is "Phill""
You'd actually get 3 tokens:
    "my name is "
    Phill (unkown token)
    ""

So we'd like to be able to do this:
    "my name is \"Phill\""

But this will give us the error: ERROR: Undefined token Phill\


By adding escape support, quotes and space characters like newlines can be used in strings
"this string \n spans lines"

We'll add support for the following escape characters:
    \n - newline
    \r - carriage return
    \t - tab
    \" - double quote
...
So that when we push it to the stack and use .s we get:
...
which is what we have currently. Words like . (stack_print) and .s (stack_display) will take care of converting the escaped newlines, returns and tabs into the appropriate form
'''
import traceback
import sys
import os.path
...
if len(args := sys.argv[1:]) > 0:
    ...
    location = ''
    # Forth source code
    src = '"my name is \\"Phill\\"" dup . .s'
    # output
    # my name is "Phill"
    # <1> "my name is \"Phill\"" ok

    # other programs
#     src = '"go fish" length .' # 7.0
#     src = '"some    string" .s' # <1> "some    string" ok
#     src = '''
# "here is some
# string that spans   multiple
# lines" .s''' # <1> "here is some\nstring that spans   multiple\nlines" ok

#     src = '"some\\nmultiline string" .s' # <1> "some\nmultiline string" ok

#     src = '''
# "'Twas brillig, and the slighty toves,
# Did gyre and gimble in the wabe:
# All mimsy were the borogroves,\nAndthe mome raths outgrabe." dup . .s'''
#     # output
#     'Twas brillig, and the slighty toves,
#     Did gyre and gimble in the wabe:
#     All mimsy were the borogroves,
#     Andthe mome raths outgrabe.
#     <1> "'Twas brillig, and the slighty toves,\nDid gyre and gimble in the wabe:\nAll mimsy were the borogroves,\nAndthe mome raths outgrabe." ok
